...
function AdoptionEdit() {
   ...
   const user = "6627930149c106305c4dad86";
   ...
   const [live_in, setLiveIn] = useState("");
   const [housing_time, setHousingTime] = useState("");
   const [pet_location_when_you_out, setPetLocationWhenYouOut] = useState("");
   ...
   useEffect(() => {
      const adoptionId = window.location.pathname.split("/")[3];
      setAdoptionId(adoptionId);
      fetchAdoption(adoptionId);
   }, []);

   const fetchAdoption = async (adoptionId) => {
      const response = await fetch(APP_URL + `/api/adoption/${adoptionId}`);
      const data = await response.json();
      setApplicantName(data.applicant_name);
      setSpouseName(data.spouse_name);
      setApplicantOccupation(data.applicant_occupation);
      setSpouseOccupation(data.spouse_occupation);
      setEmail(data.email);
      setAddress(data.address);
      setPhone(data.phone);
      setCell(data.cell);
      setNoOfChildren(data.no_of_children);
      setNoOfAdults(data.no_of_adults);
      setAnimalCruelty(data.animal_cruelty);
      setAnimalCrueltyExplanation(data.animal_cruelty_explanation);
      setHousingType(data.housing_type);
      setLiveIn(data.live_in);
      setHousingTime(data.housing_time);
      setPetLocationWhenYouOut(data.pet_location_when_you_out);
      setFenceDescription(data.fence_description);
      setNotAllowedPetExplanation(data.not_allowed_pet_explanation);
      setTravelingOften(data.traveling_often);
      setPetWhenTravel(data.pet_when_travel);
      setApplicantWorkTime(data.applicant_work_time);
      setSpouseWorkTime(data.spouse_work_time);
      setHouseholdPersonExplanation(data.household_person_explanation);
      setPetLocation(data.pet_location);
      setPetLocationExplanation(data.pet_location_explanation);
      setAnimalId(data.animal);
   }
   ...
   return (
      <div className="p-10" style={{
         ...
         <div className="w-[1000px] m-auto flex">
            <button onClick={handleEdit} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-2/3 mr-2">Update The Adoption Form</button>
            <button onClick={handleDelete} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded w-1/3 ms-2">Delete The Adoption Form</button>
         </div>
         <ToastContainer />
      </div >
